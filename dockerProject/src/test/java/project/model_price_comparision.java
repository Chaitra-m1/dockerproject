...
public class model_price_comparision {
	@Test
	public void model_price() throws IOException
	{
		 List<Integer> excel_value=new ArrayList<Integer>();
		    List<Integer> difference=new ArrayList<Integer>();
			String filepath="C:\\Users\\user\\eclipse-workspace\\dockerProject\\model_price\\model_price.xlsx";
		...
		System.out.println(excel_value.get(i));
		  ...
		  List<String> request_type = new ArrayList<String>();
		   List<Integer> request_type_model = new ArrayList<Integer>();
			...
			for(int i=0;i<90;i++)
			{
				String model_text=res.jsonPath().get("prices['Modal Price (Rs./Quintal)']["+i+"]").toString();
				
				double model_value=Double.parseDouble(model_text);
				int value=(int)model_value;
				request_type_model.add(value);
			...
			for(int i=0;i<90;i++)
			{
				...
				System.out.println(request_type_model.get(i));
			}
			...
			xlutil.setCellData("Sheet1",i,2,request_type_model.get(j++));
			...
			for(int i=0;i<90;i++)
			{
			int diff =	request_type_model.get(i)-excel_value.get(i);
			System.out.println("predicited_max_value = "+request_type_model.get(i)+"  and actual max value = "+excel_value.get(i)+"  difference is = "+diff);
			}
			System.out.println("##############33");
			for(int i=0;i<90;i++)
			{
				difference.add(request_type_model.get(i)-excel_value.get(i));
			}
